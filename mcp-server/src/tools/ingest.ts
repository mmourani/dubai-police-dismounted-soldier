...
export async function handleIngestTool(name: string, args: any) {
  if (name === 'ingest_plan') {
  ...
  if (name === 'ingest_status') {
    ...
    try {
      ...
      gaps = String((j.gaps || []).filter((g:any)=>!g.resolved).length);
    } catch {}
    async function count(d:string){ try { return (await fs.readdir(path.join(ROOT,d))).length; } catch { return 0; } }
    const text = [
